import type { DynamicPaletteT, PaletteT } from "./utils.types";
...
export const sendErrorNotification = (
  err: Error,
  params: Parameters<SendNotificationT>[0]
) => {
  sendNotification({
    ...params,
...
export const getDynamicColor = (
  ...
  return Color.dynamic(
    new Color(lightColor, opacity),
    new Color(darkColor, opacity)
  );
};

export const getDynamicGradient = ({ dark, light }: DynamicPaletteT) => {
  const startColor = Color.dynamic(
    new Color(light.bgStart),
    new Color(dark.bgStart)
  );
