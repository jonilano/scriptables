...
function getPalette(palette: { light?: string; dark?: string } = {}) {
  ...
  return {
    light: PALETTES[light ?? palette.light] ?? PALETTES.dull,
    dark: PALETTES[dark ?? palette.dark] ?? PALETTES.sandblue
  };
}
...
export function createWidget(
  ...
  const opacity =
    palette[appearence].bgStart === palette[appearence].bgEnd
      ? 1
      : GRAPH_OPACITY;
  ...
  const footerStackBottomPadding =
    (2 -
      [subtitle1, subtitle2].reduce((acc, item) => acc + (item ? 1 : 0), 0)) *
    GAP;
  ...
  if (subtitle1) {
    const subtitle1Text = footerStack.addText(
      `${subtitle1} TODAY ${subtitle2}`
    );
